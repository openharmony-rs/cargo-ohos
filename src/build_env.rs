...
/// Whether one of the variables cmake-rs takes the generator from is set.
fn cmake_generator_configured(rust: &str, rust_u: &str) -> bool {
    [
        format!("CMAKE_GENERATOR_{rust}"),
        format!("CMAKE_GENERATOR_{rust_u}"),
        "TARGET_CMAKE_GENERATOR".to_owned(),
...
// after the include keeps all of its other configuration. The SDK's `ninja`
// is rarely on PATH, so a Ninja generator is pointed at it.
            ...
            Self::SdkNotFound { tried } => {
                write!(
                    ...
                    "Could not find the OpenHarmony native SDK. Point --sdk or $OHOS_SDK_NATIVE \
                     at the `native` directory of the SDK, set $DEVECO_SDK_HOME, or download one \
                     with `cargo ohos init sdk`. Tried: {}",
                    tried.join("; ")
                )
            }
